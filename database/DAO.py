...
def get_years():
    """
        Permettere all’utente di scegliere da un menù a tendina un anno tra tutti i possibili anni in cui ci sono stati
        avvistamenti (ordinati in senso DECRESCENTE).
        """
    cnx = DBConnect.get_connection()
    result = []
    if cnx is None:
...
def get_shapes_year(anno: int):
    """
    Tutte le possibili forme, prese dalla colonna “shape” del db, relative
    agli avvistamenti nell’anno considerato (escludendo i casi in cui non è specificata nessuna forma, ordinati
    alfabeticamente).
    """
    cnx = DBConnect.get_connection()
    result = []
...
def get_nodes(year: int, shape: str):
    """
    Grafo orientato e non pesato, i cui vertici siano tutti gli avvistamenti presenti nella tabella “sighting” che siano avvenuti
...
def getEdges(year, shape, idMap):
    """
    Il grafo è un grafo diretto, ed un arco fra due avvistamenti esiste se e solo se tali avvistamenti sono avvenuti nello stesso stato.
    L’arco è uscente dall’avvistamento che è avvenuto temporalmente prima ed entrante nell’avvistamento avvenuto dopo.
    """
    cnx = DBConnect.get_connection()
    result = []
    if cnx is None:
...
# UFO-SIGHTINGS-4-07-C

@staticmethod
    def getEdges(year, shape, idMap):
"""
     Vertici siano tutti gli avvistamenti presenti nella tabella “sighting” che siano avvenuti nell’anno selezionato dall’utente e con la
forma desiderata. Un arco fra due avvistamenti esiste se e solo se tali avvistamenti sono avvenuto nello
stesso stato (colonna “state” del db). L’arco è uscente dall’avvistamento che è avvenuto in una località con
Longitudine minore ed entrante nella località a Longitudine maggiore, ed il peso è pari alla differenza in
Longitudine tra nodo di arrivo e nodo di partenza. (Esempio. Se nodo A ha Longitudine -123.0 e nodo B ha
longitudine -47 l’eventuale arco sarà diretto da A verso B ed avrà peso 76)"""
        conn = DBConnect.get_connection()
        result = []
        cursor = conn.cursor(dictionary=True)
